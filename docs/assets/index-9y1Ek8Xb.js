(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),t.credentials=e.crossOrigin===`use-credentials`?`include`:e.crossOrigin===`anonymous`?`omit`:`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=[`one`,`two`,`three`,`four`,`five`,`six`,`seven`,`eight`,`nine`,`ten`],t=[{tens:2,ones:1},{tens:1,ones:5},{tens:3,ones:4},{tens:4,ones:5},{tens:6,ones:2}],n=`abcdefghijklmnopqrstuvwxyz`,r=document.querySelector(`#app`),i=`baxter`,a=`/austin-homework/hero.png`,o=0,s=!1,c=!1,l=``,u=[],d=[],f=0,p=``,m=``,h=`tens`,g=`login`,_=0,v=!1,y=``,b=``,x=0,S=0,C={},w={},T=`spell`;function E(e){return e.length}function D(){return e.reduce((e,t)=>e+t.length,0)}function O(){try{return JSON.parse(localStorage.getItem(`austin-scores`)||`[]`)}catch{return[]}}function k(){let e=O();e.unshift({when:new Date().toLocaleString(),spell:x,spellMax:D(),maths:S,mathsMax:t.length}),localStorage.setItem(`austin-scores`,JSON.stringify(e.slice(0,20)))}function A(){return O()[0]}function j(e,t){let n=t?Math.round(e/t*5):0;return`★`.repeat(n)+`☆`.repeat(5-n)}function M(){for(let e=0;e<48;e++){let t=document.createElement(`div`);t.className=`bit`,t.style.left=Math.random()*100+`vw`,t.style.background=[`#c41e3a`,`#f4d35e`,`#2d5bff`,`#fff`][e%4],t.style.animationDelay=Math.random()*.4+`s`,document.body.appendChild(t),setTimeout(()=>t.remove(),2500)}}function N(e){let t=document.createElement(`div`);t.className=`flash-ok`,t.textContent=`Correct Well Done!`,document.body.appendChild(t),F(1400,()=>{t.remove(),e()})}function P(){d.forEach(e=>clearTimeout(e)),d=[];try{speechSynthesis.cancel()}catch{}}function F(e,t){d.push(setTimeout(t,e))}function I(t){if(v=!0,g===`spell`&&U(),F(850,()=>{v=!1,g===`spell`&&U()}),e.indexOf(t)!==-1){let e=new Audio(`/austin-homework/sounds/`+t+`.mp3`);e.playsInline=!0;let n=e.play();n&&n.catch&&n.catch(function(){})}try{speechSynthesis.cancel();let e=new SpeechSynthesisUtterance(t);e.rate=.75,speechSynthesis.speak(e)}catch{}}function L(e){return e.map(e=>({item:e,sort:Math.random()})).sort((e,t)=>e.sort-t.sort).map(({item:e})=>e)}function R(e){let t=L(n.split(``)).filter(t=>!e.includes(t)).slice(0,e.length<5?3:2);return L([...e.split(``),...t])}function z(e,t){return Array.from({length:e},()=>t===`ten`?`<span class="rod"></span>`:`<span class="cube"></span>`).join(``)}function B(){let t=e[o];P(),s=!1,c=!0,l=``,u=R(t),_=5,v=!1,U();function n(){F(1e3,()=>{--_,_>0?(U(),n()):(I(t),F(2e3,()=>{I(t),F(2e3,()=>{I(t),F(800,()=>{s=!0,c=!1,U()})})}),U())})}n()}function V(){T=`spell`,k(),g=`result`,U(),x===D()&&M()}function H(){T=`maths`,k(),g=`result`,U(),S===t.length&&M()}function U(){if(g===`login`){r.innerHTML=`
      <main class="card">
    ...
    `;return}if(g===`home`){let e=A();r.innerHTML=`
      <main class="card">
        <div class="login-wrap">
          <div class="hero-btn"><img src="${a}" alt="Austin" /></div>
        </div>
    ...
    `;return}if(g===`result`){let e=T===`spell`?x:S,n=T===`spell`?D():t.length;r.innerHTML=`
      <main class="card">
    ...
    `;return}if(g===`spell`){let t=e[o],n=s&&!c&&_===0,i=n&&l.length>0;r.innerHTML=`
      <main class="card">
        ...
        <p class="progress">Word ${o+1} of ${e.length} · ${E(t)} pts</p>
    ...
    <main class="card">
      ...
      <div class="build">
        <div class="rods">${z(n.tens,`ten`)}</div>
        <div class="ones">${z(n.ones,`one`)}</div>
      </div>
  ...
  `}function W(n,r){if(n){if(n===`logout`){g=`login`,y=``,b=``,P(),U();return}if(n===`pick-austin`){g=`pass`,y=``,b=``,U();return}if(n===`pass-letter`){y+=r,b=``,U();return}if(n===`pass-clear`){y=``,b=``,U();return}if(n===`pass-go`){y===i?(g=`home`,y=``,b=``):(b=`Try again`,y=``),U();return}if(n===`home`){g=`home`,P(),U();return}if(n===`spell`){g=`spell`,o=0,x=0,C={},B();return}if(n===`maths`){g=`maths`,f=0,S=0,w={},p=``,m=``,h=`tens`,P(),U();return}if(n===`hear`){I(e[o]);return}if(n===`letter`){if(!s||c||_>0)return;l+=r,U();return}if(n===`check-spell`){let t=e[o],n=document.querySelector(`#result`);l===t?(C[o]||(x+=E(t),C[o]=!0),n.textContent=`Correct Well Done!`,n.className=`ok`,I(`Well done`),N(()=>{o>=e.length-1?V():(o+=1,B())})):(l=``,u=R(t),U(),document.querySelector(`#result`).textContent=`Try again`,document.querySelector(`#result`).className=`no`);return}if(n===`again`){B();return}if(n===`next-word`){o>=e.length-1?V():(o+=1,B());return}if(n===`tens`){h=`tens`,U();return}if(n===`ones`){h=`ones`,U();return}if(n===`num`){h===`tens`?p=r:m=r,U();return}if(n===`check-maths`){let e=t[f],n=document.querySelector(`#result`);Number(p)===e.tens&&Number(m)===e.ones?(w[f]||(S+=1,w[f]=!0),n.textContent=`Correct Well Done!`,n.className=`ok`,I(`Well done`),N(()=>{f>=t.length-1?H():(f+=1,p=``,m=``,h=`tens`,U())})):(n.textContent=`Try again`,n.className=`no`);return}if(n===`clear`){p=``,m=``,h=`tens`,U();return}n===`next-maths`&&(f>=t.length-1?H():(f+=1,p=``,m=``,h=`tens`,U()))}}function G(e){let t=e;for(;t&&t!==r;){if(t.getAttribute&&t.getAttribute(`data-act`))return t;t=t.parentNode}return null}r.onclick=function(e){let t=G(e.target);t&&(t.className||``).indexOf(`off`)===-1&&W(t.getAttribute(`data-act`),t.getAttribute(`data-val`))},U();